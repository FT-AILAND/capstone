...
class _ExerciseCalendarPageState extends State<ExerciseCalendarPage> {
    ...
    if (user != null) {
      try {
        ...
        for (var doc in querySnapshot.docs) {
          ...
          print('Processed data: Date: $date, Workout: $workoutName, Count: $count');
  ...
  void _updateSelectedDayEvents(DateTime selectedDay) {
    final eventDate = DateTime(selectedDay.year, selectedDay.month, selectedDay.day);
    _selectedDayEvents = _eventDetails[eventDate] ?? [];
    _selectedDayEvents.sort((a, b) => (b['timestamp'] as DateTime).compareTo(a['timestamp'] as DateTime));
  }
                          ...
                          if (events.isNotEmpty) {
                            return Positioned(
                              ...
                              child: Row(
                                ...
                                children: events.map((color) => _buildMarker(color)).toList(),
                              ),
                            );
                          }
                          return null;
                        ...
                        return Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(workoutNames[entry.key] ?? entry.key, style: const TextStyle(color: Colors.white)),
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Row(
                                        children: [
                                          CircleAvatar(
                                            backgroundColor: workoutColors[event['workout_name']],
                                            radius: 8,
                                          ),
                                          const SizedBox(width: 10),
                                          Expanded(
  ...
  Widget _buildMarker(Color color) {
    String workoutName = workoutColors.entries.firstWhere((entry) => entry.value == color).key;
    return Tooltip(
